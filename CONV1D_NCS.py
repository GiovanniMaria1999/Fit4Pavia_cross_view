...
# faccio la trasposizione
    ...
    index_train = sorted(random.sample(lista_indici, dim_train))
    ...
    xtrain = tabella[index_train]
    xtest = tabella[index_test]


# normalizzo le sequenze

    #mean = xtrain.mean(dim = (0,2),keepdim = True)
    #std = xtrain.std(dim = (0,2), keepdim = True)
    ...
    for j in range(len(ytest)):
        print(ytest[j], nome_chiave_test[j])
...
# adesso costruisco la rete conv1d

# coonv1d vuole xtrain nella forma (batch size, input_channels, sequence length)
    ...
    def compute_output_size(input_length, kernel_size, stride, padding, pooling_size):
        ...
        print(conv_length)
        pooled_length = conv_length // pooling_size  # Lunghezza dopo pooling4
        print(pooled_length)
    ...
    #cartella = "modelli"
    #percorso = os.path.join(cartella, "conv1d_ncs.pth")
    #torch.save(model,percorso)
...
with open(os.path.join(cartella,'accuratezza_conv1d_ncs_test.pkl'), 'wb') as file:
    pickle.dump(lista_acc_test,file)

with open(os.path.join(cartella, 'accuratezza_conv1d_ncs_train.pkl'), 'wb') as file:
    pickle.dump(lista_acc_train, file)

with open(os.path.join(cartella,'f1_conv1d_ncs_test.pkl'), 'wb') as file:
    pickle.dump(lista_f1_test,file)

with open(os.path.join(cartella,'f1_conv1d_ncs_train.pkl'), 'wb') as file:
    pickle.dump(lista_f1_train, file)

with open(os.path.join(cartella,'confmatrix_conv1d_ncs.pkl'), 'wb') as file:
    pickle.dump(conf_matrix_conv , file)

with open(os.path.join(cartella,'conv1d_simul_id_ncs.pkl'), 'wb') as file:
    pickle.dump(array_simul_id,file)

with open(os.path.join(cartella,'conv1d_array_dim_train_ncs.pkl'), 'wb') as file:
    pickle.dump(array_dim_train_id,file)

with open(os.path.join(cartella,'conv1d_array_dim_test_ncs.pkl'), 'wb') as file:
    pickle.dump(array_dim_test_id,file)

with open(os.path.join(cartella,'conv1d_etichetta_is_ncs.pkl'), 'wb') as file:
    pickle.dump(etichetta_is_tot, file)

with open(os.path.join(cartella,'conv1d_etichetta_is_train_ncs.pkl'), 'wb') as file:
    pickle.dump(etichetta_is_tot_train, file)

with open(os.path.join(cartella,'conv1d_etichetta_is_test_ncs.pkl'), 'wb') as file:
    pickle.dump(etichetta_is_tot_test, file)

with open(os.path.join(cartella,'conv1d_lista_nomi_simulazione_ncs.pkl'), 'wb') as file:
    pickle.dump(lista_nomi_sim,file)

with open(os.path.join(cartella,'conv1d_is_pred_cc_train_ncs.pkl'), 'wb') as file:
    pickle.dump(is_pred_cc_train_tot,file)

with open(os.path.join(cartella,'conv1d_is_pred_cc_test_ncs.pkl'), 'wb') as file:
    pickle.dump(is_pred_cc_test_tot,file)

with open(os.path.join(cartella,'conv1d_item_loss_test_ncs.pkl'), 'wb') as file:
    pickle.dump(item_loss_test_tot,file)

with open(os.path.join(cartella,'conv1d_item_loss_train_ncs.pkl'), 'wb') as file:
    pickle.dump(item_loss_train_tot,file)

with open(os.path.join(cartella,'conv1d_prob_conf_train_ncs.pkl'), 'wb') as file:
    pickle.dump(lista_prob_pred_train,file)

with open(os.path.join(cartella,'conv1d_prob_conf_test_ncs.pkl'), 'wb') as file:
    pickle.dump(lista_prob_pred_test,file)

with open(os.path.join(cartella,'conv1d_ytest_ncs.pkl'), 'wb') as file:
    pickle.dump(ytest_tot,file)

with open(os.path.join(cartella,'conv1d_ytrain_ncs.pkl'), 'wb') as file:
    pickle.dump(ytrain_tot,file)

with open(os.path.join(cartella,"iperparametri_conv1d_ncs.pkl"),"wb") as file:
    pickle.dump(iper_tot, file)

with open(os.path.join(cartella,'conv1d_pred_test_ncs.pkl'), 'wb') as file:
    pickle.dump(lista_predizioni_test,file)

with open(os.path.join(cartella,"conv1d_pred_train_ncs.pkl"),"wb") as file:

...
for i in range(100):
    ...
    for paziente in paz_train:
        for index, nome in enumerate(lista_nome_vf2):
            if paziente in nome:
    ...
    for j in indici_cross_subject_train:
        ...
        train_cross_subject.append(paz)

    test_cross_subject = []
    for j in indici_cross_subject_test:
        paz = dati_skeleton_body[j]
        test_cross_subject.append(paz)
    ...
    train_cross_subject_cross_view = []
    for j in indici_cross_subject_cross_view_train:
        paz = train_cross_subject[j]
        train_cross_subject_cross_view.append(paz)

    test_cross_subject_cross_view = []
    for j in indici_cross_subject_cross_view_test:
        paz = test_cross_subject[j]
        test_cross_subject_cross_view.append(paz)
...
with open(os.path.join(cartella,'accuratezza_conv1d_mc_cv_cs_test.pkl'), 'wb') as file:
    pickle.dump(lista_acc_test,file)

with open(os.path.join(cartella, 'accuratezza_conv1d_mc_cv_cs_train.pkl'), 'wb') as file:
    pickle.dump(lista_acc_train, file)

with open(os.path.join(cartella,'f1_conv1d_mc_cv_cs_test.pkl'), 'wb') as file:
    pickle.dump(lista_f1_test,file)

with open(os.path.join(cartella,'f1_conv1d_mc_cv_cs_train.pkl'), 'wb') as file:
    pickle.dump(lista_f1_train, file)

with open(os.path.join(cartella,'confmatrix_conv1d_mc_cv_cs.pkl'), 'wb') as file:
    pickle.dump(conf_matrix , file)

with open(os.path.join(cartella,'conv1d_mc_simul_id_cv_cs.pkl'), 'wb') as file:
    pickle.dump(array_simul_id,file)

with open(os.path.join(cartella,'conv1d_mc_array_dim_train_cv_cs.pkl'), 'wb') as file:
    pickle.dump(array_dim_train_id,file)

with open(os.path.join(cartella,'conv1d_mc_array_dim_test_cv_cs.pkl'), 'wb') as file:
    pickle.dump(array_dim_test_id,file)

with open(os.path.join(cartella,'conv1d_mc_etichetta_is_sub_cv_cs.pkl'), 'wb') as file:
    pickle.dump(etichetta_is_sub_tot, file)

with open(os.path.join(cartella,'conv1d_mc_etichetta_is_sett_cv_cs.pkl'), 'wb') as file:
    pickle.dump(etichetta_is_sett_tot, file)

with open(os.path.join(cartella,'conv1d_mc_etichetta_is_train_cv_cs.pkl'), 'wb') as file:
    pickle.dump(etichetta_is_tot_train, file)

with open(os.path.join(cartella,'conv1d_mc_etichetta_is_test_cv_cs.pkl'), 'wb') as file:
    pickle.dump(etichetta_is_tot_test, file)

with open(os.path.join(cartella,'conv1d_mc_lista_nomi_simulazione_cv_cs.pkl'), 'wb') as file:
    pickle.dump(lista_nomi_sim,file)

with open(os.path.join(cartella,'conv1d_mc_is_pred_cc_train_cv_cs.pkl'), 'wb') as file:
    pickle.dump(is_pred_cc_train_tot,file)

with open(os.path.join(cartella,'conv1d_mc_is_pred_cc_test_cv_cs.pkl'), 'wb') as file:
    pickle.dump(is_pred_cc_test_tot,file)

with open(os.path.join(cartella,'conv1d_mc_item_loss_test_cv_cs.pkl'), 'wb') as file:
    pickle.dump(item_loss_test_tot,file)

with open(os.path.join(cartella,'conv1d_mc_item_loss_train_cv_cs.pkl'), 'wb') as file:
    pickle.dump(item_loss_train_tot,file)

with open(os.path.join(cartella,'conv1d_mc_prob_conf_train_cv_cs.pkl'), 'wb') as file:
    pickle.dump(lista_prob_pred_train,file)

with open(os.path.join(cartella,'conv1d_mc_prob_conf_test_cv_cs.pkl'), 'wb') as file:
    pickle.dump(lista_prob_pred_test,file)

with open(os.path.join(cartella,'conv1d_mc_ytest_cv_cs.pkl'), 'wb') as file:
    pickle.dump(ytest_tot,file)

with open(os.path.join(cartella,'conv1d_mc_ytrain_cv_cs.pkl'), 'wb') as file:
    pickle.dump(ytrain_tot,file)

with open(os.path.join(cartella,"iperparametri_conv1d_mc_cv_cs.pkl"),"wb") as file:
    pickle.dump(iperparametri, file)

with open(os.path.join(cartella,'conv1d_mc_pred_test_cv_cs.pkl'), 'wb') as file:
    pickle.dump(lista_predizioni_test,file)

with open(os.path.join(cartella,"conv1d_mc_pred_train_cv_cs.pkl"),"wb") as file:
